...
function merge(array,l,mid,m,newarr,animations){
    ...
    while(j<=m && i<mid){
        ...
        if(newarr[i]<=newarr[j]){
            animations.push([k,newarr[i]]);
            array[k++]=newarr[i++];
        }else{
            animations.push([k,newarr[j]]);
            array[k++]=newarr[j++];
        }
    }
    while(i<mid){
...
function mergeSort(array,l,m,newarr,animations){
    if(m-l>0){
        let mid=(l+m)/2;
        mid= Math.round(mid);
        mergeSort(newarr,l,mid-1,array,animations);
        mergeSort(newarr,mid,m,array,animations);
        merge(newarr,l,mid,m,array,animations);
        // console.log(array);
    }
    return
}
